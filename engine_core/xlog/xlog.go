...
type Xlog struct {
	initInfo     *conf.LogInitModel  // 初始化
	baseLog      *log.Logger         // 内置log库的处理
	logBufchan   chan *conf.LogModel // 日志信息
	closeFlag    *model.AtomicBool
	logmodelPool sync.Pool
}

// 创建日志对象
func NewXlog(info *conf.LogInitModel) bool {
	_xlog = new(Xlog)
	if _xlog == nil {
	...
	_xlog.logBufchan = make(chan *conf.LogModel, info.Volatile.LogQueueCap)
	_xlog.initInfo = info
...
func initXlog() {
	...
	_xlog.logmodelPool.New = func() interface{} {
		return new(conf.LogModel)
	}
}

// 设置日志等级并设置是否在控制台显示 目前这两个经常改变
func SetShowLogAndStartLog(restmodel conf.VolatileLogModel) bool {
	if _xlog == nil {
...
func Debug(format string, v ...interface{}) {
	...
	addLogToLogBufchan(DebugLvl, "", str)
}

//警告和错误把堆栈打出来
func Warning(format string, v ...interface{}) {
	str := fmt.Sprintf(format, v...)
	addLogToLogBufchan(WarningLvl, "", str)
}

func Error(format string, v ...interface{}) {
	str := fmt.Sprintf(format, v...)
	addLogToLogBufchan(ErrorLvl, "", xutil.GetTraceStackStr(str))
}

func DebugLog(scenename string, format string, v ...interface{}) {
	str := fmt.Sprintf(format, v...)
	addLogToLogBufchan(DebugLvl, scenename, str)
}
func WarningLog(scenename string, format string, v ...interface{}) {
	str := fmt.Sprintf(format, v...)
	addLogToLogBufchan(WarningLvl, scenename, xutil.GetTraceStackStr(str))
}

func ErrorLog(scenename string, format string, v ...interface{}) {
	str := fmt.Sprintf(format, v...)
	addLogToLogBufchan(ErrorLvl, scenename, xutil.GetTraceStackStr(str))
}

// 向log日志队列中写日志信息
func addLogToLogBufchan(loglvl uint16, scenename string, logStr string) {
	if _xlog == nil {
		fmt.Println("addLogToLogBufchan xlog is nil 日志为:", logStr)
		return
	}
...
func (xl *Xlog) writeLogToFile(lm *conf.LogModel) {
	if lm == nil {
		return
	}
...
func (xl *Xlog) newLogFile(lm *conf.LogModel) *os.File {
	// 每两个小时一个文件
